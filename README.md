...
A small Windows tray utility that makes playing **Dota 1 on RGC (Ranked Gaming Client)** comfortable. Sign for games with a hotkey without leaving your current match, get borderless Warcraft III with auto focus, restyle the game and the client. All of it lives in the system tray.
...
### 🔔 Game alerts
Sound and Windows notification the moment your game launches, so you can tab away while waiting for signs.
...
- **Smooth graphics**: DPI override and fullscreen optimization fix
...
1. Download `W3UtilSetup.exe` from [Releases](https://github.com/wekz/w3-util/releases/latest) and run it (admin prompt, needed because RGC runs elevated).
   - SmartScreen may warn since the exe is unsigned: click *More info*, then *Run anyway*.
